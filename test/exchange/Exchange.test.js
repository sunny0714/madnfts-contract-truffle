...
const { ETH, ERC20, ERC721, ERC1155, ORDER_DATA_V1, ORDER_DATA_V2, TO_MAKER, TO_TAKER, PROTOCOL, ROYALTY, ORIGIN, PAYOUT, enc, id } = require("../assets");
...
contract("Exchange, sellerFee + buyerFee =  6%,", accounts => {
  ...
  let royaltiesRegistry;
    ...
    await testing.setFeeReceiver(t1.address, protocol);
 		/*ERC721 */
 		erc721 = await TestERC721.new("MadNft", "MAD", "https://ipfs.madnfts.com");
		/*ERC1155V2*/
		erc1155_v1 = await ERC1155_V1.new("https://ipfs.madnfts.com");
		erc1155_v1.initialize();
		/*ERC721_V1 */
 		erc721V1 = await ERC721_V1.new("MadNft", "MAD", "https://ipfs.madnfts.com");
	...
	describe("Do matchOrders(), orders dataType == V1", () => {
		...
		it("From ERC20(DataV1) to ERC1155(RoyalytiV2, DataV1) Protocol, Origin fees, Royalties ", async () => {
			...
			assert.equal(await erc1155_v1.balanceOf(accounts[1], erc1155TokenId1), 7);
			assert.equal(await erc1155_v1.balanceOf(accounts[2], erc1155TokenId1), 3);
			assert.equal(await t1.balanceOf(protocol), 6);
		})

		async function prepare20DV1_1155V2Orders(t1Amount = 120, t2Amount = 10) {
			await t1.mint(accounts[1], t1Amount);
			await  erc1155_v1.mint(accounts[2], erc1155TokenId1, [], t2Amount);
			await t1.approve(erc20TransferProxy.address, 10000000, { from: accounts[1] });
			await  erc1155_v1.setApprovalForAll(transferProxy.address, true, {from: accounts[2]});
			...
			await royaltiesRegistry.setRoyaltiesByToken(erc1155_v1.address, [[accounts[6], 1000], [accounts[7], 500]]); //set royalties by token
			const left = Order(accounts[1], Asset(ERC20, enc(t1.address), 100), ZERO, Asset(ERC1155, enc( erc1155_v1.address, erc1155TokenId1), 7), 1, 0, 0, ORDER_DATA_V1, encDataLeft);
			const right = Order(accounts[2], Asset(ERC1155, enc( erc1155_v1.address, erc1155TokenId1), 7), ZERO, Asset(ERC20, enc(t1.address), 100), 1, 0, 0, ORDER_DATA_V1, encDataRight);
			return { left, right }
		}
		...
		async function prepare1155V1_20DV1Orders(t1Amount = 120, t2Amount = 10) {
			...
			await royaltiesRegistry.setRoyaltiesByToken(erc1155_v1.address, [[accounts[6], 1000], [accounts[7], 500]]); //set royalties by token
			const left = Order(accounts[2], Asset(ERC1155, enc( erc1155_v1.address, erc1155TokenId1), 7), ZERO, Asset(ERC20, enc(t1.address), 100), 1, 0, 0, ORDER_DATA_V1, encDataLeft);
			const right = Order(accounts[1], Asset(ERC20, enc(t1.address), 100), ZERO, Asset(ERC1155, enc( erc1155_v1.address, erc1155TokenId1), 7), 1, 0, 0, ORDER_DATA_V1, encDataRight);
			return { left, right }
		}
    	...
    	await expectThrow(
      	testing.matchOrders(left, "0x", right, await getSignature(right, accounts[1]), { from: accounts[2], value: 300, gasPrice: 0 })
			...
			/*comments for k.shcherbakov@madnfts.com to show mechanism*/
...
//			eventEmitted  - срабатывает по нескольким transfer, для фиксации ошибки нужно чтоб все трансферы завалились
		...
		it("From ERC1155(DataV2) to ETH(DataV1) Protocol, check emit ", async () => {
			...
			await erc1155_v1.mint(seller, erc1155TokenId1, [[sellerRoyaltiy, 1000]], 10);
    	await erc1155_v1.setApprovalForAll(transferProxy.address, true, {from: seller});
			...
			const left = Order(seller, Asset(ERC1155, enc(erc1155_v1.address, erc1155TokenId1), 5), ZERO, Asset(ETH, "0x", 200), 1, 0, 0, ORDER_DATA_V1, encDataLeft);
			const right = Order(buyer, Asset(ETH, "0x", 200), ZERO, Asset(ERC1155, enc(erc1155_v1.address, erc1155TokenId1), 5), 1, 0, 0, ORDER_DATA_V1, encDataRight);
...
//			eventEmitted  - срабатывает по нескольким transfer, для фиксации ошибки нужно чтоб все трансферы завалились
  ...
  describe("matchOrders, orderType = V2", () => {
    it("should correctly calculate make-side fill for isMakeFill = true ", async () => {
      ...
      await erc1155_v1.mint(seller, erc1155TokenId1, [], 200);
      await erc1155_v1.setApprovalForAll(transferProxy.address, true, { from: seller });
  
      const encDataLeft = await encDataV2([[], [], true]);
      const encDataRight = await encDataV2([[], [], false]);
  
      const left = Order(seller, Asset(ERC1155, enc(erc1155_v1.address, erc1155TokenId1), 200), ZERO, Asset(ETH, "0x", 1000), 1, 0, 0, ORDER_DATA_V2, encDataLeft);
      const right = Order(buyer, Asset(ETH, "0x", 500), ZERO, Asset(ERC1155, enc(erc1155_v1.address, erc1155TokenId1), 100), 1, 0, 0, ORDER_DATA_V2, encDataRight);
      ...
      assert.equal(await erc1155_v1.balanceOf(buyer, erc1155TokenId1), 100);
      assert.equal(await erc1155_v1.balanceOf(seller, erc1155TokenId1), 100);
  
      const leftOrderHash = await libOrder.hashKey(left);
      const test_hash = await libOrder.hashV2(seller, Asset(ERC1155, enc(erc1155_v1.address, erc1155TokenId1), 200), Asset(ETH, "0x", 1000), 1, encDataLeft)
      ...
      const left1 = Order(seller, Asset(ERC1155, enc(erc1155_v1.address, erc1155TokenId1), 200), ZERO, Asset(ETH, "0x", 600), 1, 0, 0, ORDER_DATA_V2, encDataLeft);
      const right1 = Order(buyer1, Asset(ETH, "0x", 300), ZERO, Asset(ERC1155, enc(erc1155_v1.address, erc1155TokenId1), 100), 1, 0, 0, ORDER_DATA_V2, encDataRight);
      ...
      assert.equal(await erc1155_v1.balanceOf(buyer1, erc1155TokenId1), 100);
      assert.equal(await erc1155_v1.balanceOf(seller, erc1155TokenId1), 0);
    })
    ...
    it("should correctly calculate take-side fill for isMakeFill = false ", async () => {
      ...
      const left1 = Order(seller, Asset(ERC1155, enc(erc1155_v1.address, erc1155TokenId1), 200), ZERO, Asset(ETH, "0x", 2000), 1, 0, 0, ORDER_DATA_V2, encDataLeft);
      const right1 = Order(buyer1, Asset(ETH, "0x", 1000), ZERO, Asset(ERC1155, enc(erc1155_v1.address, erc1155TokenId1), 100), 1, 0, 0, ORDER_DATA_V2, encDataRight);
